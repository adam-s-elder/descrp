test_that("marginal_discrete returns per-output structure", {
  ...
  expect_named(out$table,           c("output", "exclude", "info"))
  expect_named(out$table_collapsed, c("output", "exclude", "info"))
})

test_that("table_collapsed is NULL when <= 10 categories", {
  df <- data.frame(x = sample(letters[1:5], 100, replace = TRUE))
  out <- marginal_discrete(df, "x")
  expect_null(out$table_collapsed$output)
})
...
test_that("table_collapsed is produced when > 10 categories", {
  ...
  out <- marginal_discrete(df, "x")
  expect_s3_class(out$table$output,           "knitr_kable")
  expect_s3_class(out$table_collapsed$output, "knitr_kable")
})
...
test_that("table_collapsed contains '< 5%' row", {
  ...
  df <- data.frame(x = sample(letters, 500, replace = TRUE))
  out <- marginal_discrete(df, "x")
  tbl_text <- paste(out$table_collapsed$output, collapse = "\n")
...
test_that("counts are comma-formatted for large numbers", {
  ...
  out <- marginal_discrete(data.frame(x = x), "x")
  tbl_text <- paste(out$table$output, collapse = "\n")
...
test_that("info$summary_type is discrete_marginal", {
  df <- data.frame(x = c("a", "b", "c"))
  out <- marginal_discrete(df, "x")
  expect_equal(out$table$info$summary_type, "discrete_marginal")
  expect_equal(out$table$info$covariate, "x")
})

test_that("all excludes are NULL for discrete marginal", {
  df <- data.frame(x = sample(letters, 500, replace = TRUE))
  out <- marginal_discrete(df, "x")
  expect_null(out$table$exclude)
  expect_null(out$table_collapsed$exclude)
})
