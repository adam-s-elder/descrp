test_that("marginal_discrete returns output/excludes/info structure", {
  df <- data.frame(x = sample(letters[1:5], 100, replace = TRUE))
  out <- marginal_discrete(df, "x")
  expect_type(out, "list")
  expect_named(out, c("output", "excludes", "info"))
  expect_named(out$output,   c("table", "table_collapsed"))
  expect_named(out$excludes, c("table", "table_collapsed"))
})

test_that("table_collapsed is NULL when <= 10 categories", {
  df <- data.frame(x = sample(letters[1:5], 100, replace = TRUE))
  out <- marginal_discrete(df, "x")
  expect_null(out$output$table_collapsed)
})
...
test_that("table_collapsed is produced when > 10 categories", {
  ...
  out <- marginal_discrete(df, "x")
  expect_s3_class(out$output$table,           "knitr_kable")
  expect_s3_class(out$output$table_collapsed, "knitr_kable")
})
...
test_that("table_collapsed contains '< 5%' row", {
  ...
  df <- data.frame(x = sample(letters, 500, replace = TRUE))
  out <- marginal_discrete(df, "x")
  tbl_text <- paste(out$output$table_collapsed, collapse = "\n")
...
test_that("counts are comma-formatted for large numbers", {
  ...
  out <- marginal_discrete(data.frame(x = x), "x")
  tbl_text <- paste(out$output$table, collapse = "\n")
...
test_that("info$summary_type is discrete_marginal", {
  df <- data.frame(x = c("a", "b", "c"))
  out <- marginal_discrete(df, "x")
  expect_equal(out$info$summary_type, "discrete_marginal")
...
test_that("all excludes are NULL for discrete marginal", {
  df <- data.frame(x = sample(letters, 500, replace = TRUE))
  out <- marginal_discrete(df, "x")
  expect_null(out$excludes$table)
  expect_null(out$excludes$table_collapsed)
})
